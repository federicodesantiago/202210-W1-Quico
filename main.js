...
questionScreen.innerHTML =
    'Vas a empezar una fantástica partida de Pasapalabra. Prepárate para fliparlo. Pero lo primero es lo primero. ¿Cómo te llamas?';
...
const welcomePlayer = () => {
    let namePlayer = document.getElementById('answerInput').value;
...
const checkAnswer = () => {
    if (answer === questions[numQuestions][i].answer) {
        ...
        document.getElementById(colorLetra).style.background =
            'linear-gradient(rgb(0,150,0), rgb(0,255,0))';
        ...
        colorLetra = questions[numQuestions][i].htmlID;
        document.getElementById(colorLetra).style.background =
            'linear-gradient(rgb(150,0,0), rgb(255,0,0))';
    }
    return scoreRight, scoreWrong, i;
};
